...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
      ...
      (0 == std::min((int)mxGetM(prhs[6]), (int)mxGetN(prhs[6]))))
  {
    ...
    plhs[2] = mxCreateDoubleMatrix(6, 0, mxREAL);
    plhs[3] = mxCreateDoubleMatrix(6, 0, mxREAL);
  ...
  // Allocate outputs
  int i, maxPoints = 0;
  double minxq = 0.0, maxxq = 0.0, minyq = 0.0, maxyq = 0.0, minzq = 0.0,
         maxzq = 0.0;
  if (SWpoints == 0)
  ...
  plhs[2] = mxCreateDoubleMatrix(6, maxPoints, mxREAL);
  plhs[3] = mxCreateDoubleMatrix(6, maxPoints, mxREAL);
...
#pragma omp parallel num_threads(maxNumThreads)
  {
    int _i, _ig, _point;
    double _w, _Jdet, _X, _Y, _Z, _radius2, _tradius2;
    double _U[3], _F[9], _C[6], _E[6], _S[6], _CD[6][6], _tcoord[4], _Ce[3],
        _matconst[8]; // use total Lagrangian formulation
    ...
    double **_Be = new double *[6]; // element's matrix of derivatives of basis
                                    // functions (Green-Lagrange strain)
    for (_i = 0; _i < 6; _i++)
      _Be[_i] = new double[3 * np];
    double **_Bf = new double *[9]; // element's matrix of derivatives of basis
    ...
    int (*_getWSC)(
        const double matconst[8], const double C[6], double &w, double S[6],
...
#pragma omp for
      ...
      { // loop over all elements
        ...
        if (intersect3d(_minxe, _maxxe - _minxe, _minye, _maxye - _minye,
                        _minze, _maxze - _minze, minxq, maxxq - minxq, minyq,
          ...
          for (_point = 0; _point < npoints; _point++)
          {
                ...
                1.25 * _radius2 + TOL_g)
            {
              ...
              { // otherwise it is NAN
                ...
                _getBe(np, _tcoord, _x, _y, _z, _uel, _Jdet, _Be, _Bf, _F);
                getC3d(_F, _C);
                getE3d(_F, _E);
                _getWSC(_matconst, _C, _w, _S, _CD);
                _natural2physical(np, _tcoord, _x, _y, _z, _X, _Y, _Z);
                ...
                // Allocate U, F, P, X, Y directly to outputs to guarantee
                // proper ordering of all fields
#pragma omp atomic
                prW[_point] += _w;
#pragma omp atomic
                prX[_point] += _X;
...
#pragma omp atomic
                ...
                {
#pragma omp atomic
                  prE[_point * 6 + _i] += _E[_i];
#pragma omp atomic
                  prS[_point * 6 + _i] += _S[_i];
                }
...
#pragma omp single
      {
        for (_point = 0; _point < npoints; _point++)
        {
          ...
          { // divide by multiplicity if point is located within
            ...
            for (_i = 0; _i < 6; _i++)
            {
              prE[6 * _point + _i] /= double(multiplicity[_point]);
              prS[6 * _point + _i] /= double(multiplicity[_point]);
            }
          }
        }
      }
...
#pragma omp atomic
          ...
          {
#pragma omp atomic
            prE[tr * abs(ngauss) * 6 + 6 * _ig + _i] += _E[_i];
#pragma omp atomic
            prS[tr * abs(ngauss) * 6 + 6 * _ig + _i] += _S[_i];
          }
        }
      }
    }
