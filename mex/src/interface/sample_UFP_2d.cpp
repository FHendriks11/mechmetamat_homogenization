...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
      ...
      (0 == std::min((int)mxGetM(prhs[6]), (int)mxGetN(prhs[6]))))
  {
    ...
    plhs[2] = mxCreateDoubleMatrix(4, 0, mxREAL);
    plhs[3] = mxCreateDoubleMatrix(4, 0, mxREAL);
  ...
  { // interpolate data at query points prx, pry
    ...
    maxyq += TOL_g;
           ...
           SWpoints == 2) // interpolate data at zero natural coordinates (midpoints of elements) or at all Gauss points
  ...
  plhs[2] = mxCreateDoubleMatrix(4, maxPoints, mxREAL);
  plhs[3] = mxCreateDoubleMatrix(4, maxPoints, mxREAL);
...
#pragma omp parallel num_threads(maxNumThreads)
  {
    int _i, _ig, _point;
    double _Wd, _Jdet, _X, _Y, _radius2, _tradius2, _matconst[8];
    double _U[2], _F[4], _P[4], _D[4][4], _tcoord[3], _Ce[2]; // use total Lagrangian formulation
...
#pragma omp atomic
          ...
          for (_point = 0; _point < npoints; _point++)
          {
                ...
                1.25 * _radius2 + TOL_g)
            {
              ...
              _physical2natural(np, _X, _Y, _x, _y, TOL_g, // inputs
                                _tcoord);                  // outputs

              if (_tcoord[0] == _tcoord[0])
              { // otherwise it is NAN

                // Get deformation gradient, its determinant and inverse, right
                // and left Cauchy-Green tensors, and invariants of the righ
                // Cauchy-Green tensor
                ...
                _natural2physical(np, _tcoord, _x, _y, // inputs
                                  _X, _Y);             // outputs
...
#pragma omp atomic
                ...
                {
#pragma omp atomic
                  prF[4 * _point + _i] += _F[_i];
#pragma omp atomic
                  prP[4 * _point + _i] += _P[_i];
                }
...
#pragma omp single
      {
        for (_point = 0; _point < npoints; _point++)
        {
          ...
          { // divide by multiplicity if point is located within
            ...
            for (_i = 0; _i < 4; _i++)
            {
              prF[4 * _point + _i] /= double(multiplicity[_point]);
              prP[4 * _point + _i] /= double(multiplicity[_point]);
            }
          }
        }
      }
...
#pragma omp atomic
        ...
        for (_ig = 0; _ig < abs(ngauss); _ig++)
        {
          ...
          // Get deformation gradient, its determinant and inverse, right and
          // left Cauchy-Green tensors, and invariants of the righ Cauchy-Green
          // tensor
          getF2d(_Be, _uel, np, _F);
...
#pragma omp atomic
          ...
          {
#pragma omp atomic
            prF[tr * abs(ngauss) * 4 + 4 * _ig + _i] += _F[_i];
#pragma omp atomic
            prP[tr * abs(ngauss) * 4 + 4 * _ig + _i] += _P[_i];
          }
        }
      }
    }
