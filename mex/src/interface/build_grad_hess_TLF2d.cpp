...
#pragma omp parallel num_threads(maxNumThreads)
	{
		...
		double _F[4], _P[4], _D[4][4], _tcoord[3], _matconst[8]; // use total Lagrangian formulation casted in F and P
		...
		int (*_getBe)(const int np, const double tcoord[3], const double *x, const double *y, double &Jdet, double **Be) = NULL; // pointer to Be functions
		int (*_getWPD)(const double matconst[8], const double F[4], double &Wd, double P[4], double D[4][4]) = NULL;			 // pointer to constitutive law functions
		if (np == 3 || np == 6 || np == 10)
		{

			// Choose Gauss integration rule
			if (ngauss == 1 || ngauss == 3 || ngauss == -3 || ngauss == 4 || ngauss == 6 || ngauss == 7)
...
#pragma omp atomic
		...
		{
			// Choose Gauss integration rule
			if (ngauss == 1 || ngauss == 4 || ngauss == 9 || ngauss == 16 || ngauss == 25)
...
#pragma omp atomic
			...
			for (_ig = 0; _ig < abs(ngauss); _ig++)
			{
				...
				// Get matrix of basis function derivatives
				_getBe(np, _tcoord, _x, _y, _Jdet, _Be);

				// Get deformation gradient, its determinant and inverse, right and left
				// Cauchy-Green tensors, and invariants of the righ Cauchy-Green tensor
				...
				// Integrate element's gradient f = f + alpha(ig)*(B')*P(:)*Jdet*h
				for (_i = 0; _i < 4; _i++)
					for (_j = 0; _j < 2 * np; _j++)
						_fe[_j] += _alpha[_ig] * _thickness * _Jdet * _Be[_i][_j] * _P[_i];

				// Integrate element's Hessian K = K + alpha(ig)*(B')*D*B*Jdet*h
				for (_i = 0; _i < 2 * np; _i++)
					for (_j = 0; _j < 2 * np; _j++)
						for (_k = 0; _k < 4; _k++)
							for (_l = 0; _l < 4; _l++)
								_Ke[_i][_j] += _alpha[_ig] * _Jdet * _thickness * _Be[_k][_i] * _D[_k][_l] * _Be[_l][_j];
			}
