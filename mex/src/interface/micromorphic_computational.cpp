...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  plhs[10] = mxCreateDoubleMatrix(4, ndof, mxREAL);
  plhs[11] = mxCreateDoubleMatrix(2 * nmodes, ndof, mxREAL);
  plhs[12] = mxCreateDoubleMatrix(nmodes, ndof, mxREAL);
  //   plhs[13] = mxCreateDoubleMatrix(ndof, ndof, mxREAL);
  // Get pointers
  ...
  double *prC0wBB = mxGetPr(plhs[10]);
  double *prCiwBB = mxGetPr(plhs[11]);
  double *prCiwNB = mxGetPr(plhs[12]);
  //   double *prCwwBB = mxGetPr(plhs[13]);
...
#pragma omp parallel num_threads(maxNumThreads)
  {
    ...
    double **_DBe = new double *[4];
    double **_C0wBB = new double *[4];
    for (_i = 0; _i < 4; _i++)
    {
      ...
      _DBe[_i] = new double[2 * np];
      _C0wBB[_i] = new double[ndof];
    }
    ...
    double **_CiwBB = new double *[2 * nmodes];
    double **_pDBe = new double *[2 * nmodes];
      ...
      for (_ig = 0; _ig < abs(ngauss); _ig++)
      {
        ...
        for (_h = 0; _h < nmodes; _h++)
          for (_i = 0; _i < 2; _i++)
            for (_j = 0; _j < 2; _j++)
              for (_l = 0; _l < 2; _l++)
                _C0iBB[_i + 2 * _j][_l + 2 * _h] += _alpha[_ig] * _thickness * _Jdet *
                                                    (_Dp[_i + 2 * _j][_l + 2 * _h] + _Dgp[_i + 2 * _j][_h] * _X[_l]);
        ...
        // Integrate homogenized stifness C0wBB = C0wBB + alpha(ig)*Jdet*h*D*Be
        for (_j = 0; _j < 2 * np; _j++)
          for (_k = 0; _k < 4; _k++)
            for (_l = 0; _l < 4; _l++)
              _DBe[_k][_j] += _D[_k][_l] * _Be[_l][_j];
        for (_k = 0; _k < 4; _k++)
          for (_i = 0; _i < 2 * np; _i++)
            _C0wBB[_k][_cod[_i] - 1] += _alpha[_ig] * _Jdet * _thickness * _DBe[_k][_i];

        // Integrate homogenized stiffness CiwBB = CiwBB + alpha(ig)*Jdet*h*(phi.D+X*gradphi:D)*Be
        for (_h = 0; _h < nmodes; _h++)
          for (_i = 0; _i < 2; _i++)
            for (_j = 0; _j < 2; _j++)
              for (_k = 0; _k < 2; _k++)
                for (_l = 0; _l < 2; _l++)
                  for (_g = 0; _g < 2 * np; _g++)
                  {
                    _pDBe[_j + 2 * _h][_g] += _phi[_i + 2 * _h] * _D[_i + 2 * _j][_k + 2 * _l] * _Be[_k + 2 * _l][_g];
                    _gpDBe[_h][_g] += _gradphi[_i + 2 * _j + 4 * _h] * _D[_i + 2 * _j][_k + 2 * _l] * _Be[_k + 2 * _l][_g];
                  }
        for (_h = 0; _h < nmodes; _h++)
          for (_j = 0; _j < 2; _j++)
            for (_g = 0; _g < 2 * np; _g++)
              _CiwBB[_j + 2 * _h][_cod[_g] - 1] += _alpha[_ig] * _Jdet * _thickness *
                                                   (_pDBe[_j + 2 * _h][_g] + _X[_j] * _gpDBe[_h][_g]);

        // Integrate homogenized stiffness CiwNB = CiwNB + alpha(ig)*Jdet*h*(gradphi:D)*Be
        for (_h = 0; _h < nmodes; _h++)
          for (_g = 0; _g < 2 * np; _g++)
            _CiwNB[_h][_cod[_g] - 1] += _alpha[_ig] * _Jdet * _thickness * _gpDBe[_h][_g];
      }
...
#pragma omp atomic
      ...
      // Allocate homogenized stiffness C0wBB
      for (_i = 0; _i < 4; _i++)
      {
        for (_j = 0; _j < ndof; _j++)
        {
#pragma omp atomic
          prC0wBB[_i + 4 * _j] += _C0wBB[_i][_j];
        }
      }

      // Allocate homogenized stiffness CiwBB
      for (_i = 0; _i < 2 * nmodes; _i++)
      {
        for (_j = 0; _j < ndof; _j++)
        {
#pragma omp atomic
          prCiwBB[_i + (2 * nmodes) * _j] += _CiwBB[_i][_j];
        }
      }

      // Allocate homogenized stiffness CiwNB
      for (_i = 0; _i < nmodes; _i++)
      {
        for (_j = 0; _j < ndof; _j++)
        {
#pragma omp atomic
          prCiwNB[_i + nmodes * _j] += _CiwNB[_i][_j];
        }
      }
    }
