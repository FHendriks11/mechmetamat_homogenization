...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	double TOL_g = mxGetScalar(prhs[4]);
	...
	// Test dimensions
	if (ndim != 2)
		mexErrMsgTxt("p matrix is of wrong dimensionality (data for a 2d mesh expected).");
...
#pragma omp parallel num_threads(maxNumThreads)
	{
		int _i, _point;
		double _X, _Y, _radius2, _tradius2, _tcoord[3], _Ce[2];
		...
		{ // loop over all elements
							...
							minxq, maxxq - minxq, minyq, maxyq - minyq))
			{
				...
				for (_point = 0; _point < npoints; _point++)
				{
					...
					// If pixel is side bounding circle
