// micromorphic_stress_closed: compute homogenized quantities for the micromorphic
// formulation corresponding to an ar arbitrary amount of modes nmode and close-form formulation
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	double *prgradv0 = mxGetPr(prhs[4]);
	int nmodes = (int)mxGetScalar(prhs[5]);
	double *prvi = mxGetPr(prhs[6]);
	double *prgradvi = mxGetPr(prhs[7]);
	double *prphi = mxGetPr(prhs[8]);
	...
	if (nmodes != (int)mxGetN(prhs[6]) || nmodes != (int)mxGetN(prhs[7]) || nmodes != (int)mxGetN(prhs[7]))
		mexErrMsgTxt("nmodes is incosistent. Check inputs for vi, grad(vi), and phi.");

	// Test the number of dofs
	if (ndof != (int)mxGetM(prhs[8]))
...
#pragma omp parallel num_threads(maxNumThreads)
	{
		...
		double *_phiel = new double[nmodes * 2 * np]; // displacements associated with given element
...
#pragma omp for
		...
		{ // loop over all elements
			...
			for (_i = 0; _i < np; _i++)
			{
				_cod[2 * _i + 0] = 2 * _idel[_i] - 1;
				_cod[2 * _i + 1] = 2 * _idel[_i] - 0;
			}
			...
			for (_ig = 0; _ig < abs(ngauss); _ig++)
			{
				...
				for (_i = 0; _i < nmodes; _i++)
					for (_j = 0; _j < 2; _j++)
						for (_k = 0; _k < 2; _k++)
							for (_l = 0; _l < 2 * np; _l++)
								_gradphi[4 * _i + _j + 2 * _k] += _Be[_j + 2 * _k][_l] * _phiel[2 * np * _i + _l];

				// Get complete deformation gradient _F = grad(v0) + sum_i [ grad(v_i)*phi_i +
				// v_i*grad(phi_i) ]
				for (_i = 0; _i < 4; _i++)
					_F[_i] = prgradv0[_i];
				for (_i = 0; _i < nmodes; _i++)
					for (_j = 0; _j < 2; _j++)
						for (_k = 0; _k < 2; _k++)
							_F[_j + 2 * _k] += prgradvi[2 * _i + _k] * _phi[2 * _i + _j] +
											   prvi[_i] * _gradphi[4 * _i + _j + 2 * _k];
				_F[0] += 1.0; // add identity I
				_F[3] += 1.0;
				...
				for (_i = 0; _i < nmodes; _i++)
					for (_j = 0; _j < 2; _j++)
						_Ri[_j][_i] += _alpha[_ig] * _thickness * _Jdet * (_tRi[_j][_i]);
				...
				for (_h = 0; _h < nmodes; _h++)
					for (_i = 0; _i < 2; _i++)
						for (_j = 0; _j < 2; _j++)
							for (_l = 0; _l < 2; _l++)
								_C0iBB[_i + 2 * _j][_l + 2 * _h] += _alpha[_ig] * _thickness * _Jdet * (_Dp[_i + 2 * _j][_l + 2 * _h]);
				...
				for (_g = 0; _g < nmodes; _g++)
					for (_h = 0; _h < nmodes; _h++)
						for (_j = 0; _j < 2; _j++)
							for (_l = 0; _l < 2; _l++)
								_CiiBB[_j + 2 * _h][_l + 2 * _g] += _alpha[_ig] * _thickness * _Jdet * (_pDp[_j + 2 * _h][_l + 2 * _g]);

				// Integrate homogenized stiffness CiiBN = CiiBN + alpha(ig)*Jdet*h*(phi.D:gradphi + X*gradphi:D:gradphi)
				for (_h = 0; _h < nmodes; _h++)
					for (_g = 0; _g < nmodes; _g++)
						for (_j = 0; _j < 2; _j++)
							_CiiBN[_j + 2 * _h][_g] += _alpha[_ig] * _thickness * _Jdet * (_pDgp[_j + 2 * _h][_g]);
