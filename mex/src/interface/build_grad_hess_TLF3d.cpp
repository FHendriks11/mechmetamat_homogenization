...
#pragma omp parallel num_threads(maxNumThreads)
  {
    int _i, _j, _k, _l, _ig;
    double _Wd, _W, _Jdet;
    double _F[9], _P[9], _D[9][9], _tcoord[4], _matconst[8]; // use total Lagrangian formulation
    ...
    double **_Be = new double *[9]; // element's matrix of derivatives of basis functions
    for (_i = 0; _i < 9; _i++)
    ...
    int (*_getBe)(const int np, const double tcoord[4], const double *x, const double *y, const double *z, double &Jdet, double **Be) = NULL; // pointer to Be functions
    int (*_getWPD)(const double matconst[8], const double F[9], double &Wd, double P[9], double D[9][9]) = NULL;                              // pointer to constitutive law functions
    if (np == 4 || np == 10)
    {
...
#pragma omp atomic
      ...
      for (_ig = 0; _ig < abs(ngauss); _ig++)
      {
        ...
        _W += _alpha[_ig] * _Jdet * _Wd;

        // Integrate element's gradient f = f + alpha(ig)*(B')*P(:)*Jdet*h
        for (_i = 0; _i < 9; _i++)
          for (_j = 0; _j < 3 * np; _j++)
            _fe[_j] += _alpha[_ig] * _Jdet * _Be[_i][_j] * _P[_i];
        ...
        for (_i = 0; _i < 3 * np; _i++)
          for (_j = 0; _j < 3 * np; _j++)
            for (_k = 0; _k < 9; _k++)
              for (_l = 0; _l < 9; _l++)
                _Ke[_i][_j] += _alpha[_ig] * _Jdet * _Be[_k][_i] * _D[_k][_l] * _Be[_l][_j];
      }
